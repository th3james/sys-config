...
# Script to append specific prompt files to codex.md in the current directory.
#
# Usage:
# 1. Save this file, for example, as `add_codex_instruction.fish` in a directory in your PATH,
...
#    `chmod +x add_codex_instruction.fish`
# 3. Then you can run it like:
#    `add_codex_instruction python general` (if in PATH or as a function)
#    OR
#    `./path/to/add_codex_instruction.fish python general` (if running as a direct script)

function add_codex_instruction
    ...
    set output_file_basename "codex.md"
    ...
    if test (count $argv) -eq 0
        echo "Usage: add_codex_instruction <name1> [<name2> ...]"
        echo "Example: add_codex_instruction python general"
    ...
    for instruction_name in $argv
        ...
        if test -f "$source_file"
            # If codex.md already exists and has content, add a blank line for separation
